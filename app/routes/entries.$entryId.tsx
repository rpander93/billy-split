import { type ActionFunctionArgs, type LoaderFunctionArgs, type MetaFunction, redirect } from "@remix-run/node";
...
import type { OnlineSubmittedBill } from "~/types";
...
export default function EntryPage() {
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    submit({ ...entries, items: selection }, { method: "post", encType: "application/json" });
  };
  ...
  const remainingAmount = useMemo(
    () => calculate.remaining(bill, totalAmount, selectionAmount),
    [bill, totalAmount, selectionAmount]
  );
    ...
    <Form autoComplete="off" method="POST" onSubmit={handleSubmit}>
      <Box flexDirection="column" justifyContent="center" rowGap={2}>
        <Box flexDirection="column" rowGap={1}>
          <Logo />
          <Typography>
            Select the items that belong to you. Record your payment using the provided payment method.
          </Typography>
        </Box>
        ...
        <ReceiptBox>
          <Box alignSelf="center" flexDirection="column" rowGap={1}>
            <Typography textAlign="center">
              {bill.name}
              <a
                href={`/entries/${bill.share_code}/open`}
                target="_blank"
          ...
          <ButtonGroup size="sm">
            <Button hover={view === "items"} onClick={() => setView("items")}>
              View bill items
            </Button>
            <Button hover={view === "payments"} onClick={() => setView("payments")}>
              View payments
            </Button>
          </ButtonGroup>
          ...
          <Fragment key={element.index}>
            <Box flexDirection="column" rowGap={1}>
              ...
              <Box
                alignItems="center"
                flexDirection="row"
                justifyContent={element.payment_items.length > 0 ? "space-between" : "flex-end"}
              >
              ...
              <Fragment key={item.index ?? index}>
                <Box flexDirection="column">
                  <Box alignItems="center" flexDirection="row" columnGap={4}>
                    ...
                    <Typography marginInlineStart="auto">{formatCurrency(totalAmount, bill.currency)}</Typography>
                  </Box>
                    ...
                    {item.line_items.map((element) => {
                      ...
                      return (
                        <Box key={element.line_item_index}>
                          ...
                          <Typography flex={9 / 10} fontSize="sm">
                            {y!.description}
                          </Typography>
                        </Box>
                      );
